...
def err_list():
	...
	err_500 = Sdata.objects.all().filter(timestamp__range=(date_from,date_to)).filter(code='500').order_by('method_version')
	err_200 = Sdata.objects.all().filter(timestamp__range=(date_from,date_to)).exclude(log_code='1').order_by('method_version')
	err_list = []
...
def do_contents():
	...
	# 500的部分
	html_string11 = "<table border=1 width=100%><tr style='background-color:cadetblue'><th>HttpCode</th><th>Name</th><th>Method</th><th>ProductId</th><th>URL</th></tr>\n\r"
	html_string111 = ""
	...
	#200返回错误的部分
	html_string22 = "<table border=1 width=100%><tr style='background-color:cadetblue'><th>HttpCode</th><th>Name</th><th>Method</th><th>LogCode</th><th>debugMsg</th><th>Error</th><th>Message</th><th>ProductId</th><th>URL</th></tr>\n\r"
	html_string222 = ''
		...
		for x in my_lists[1]:
			...
			for z in temp_list:
				if str(z).startswith('productId'):
					...
					pId = 'null'
		# 拼html
			html_string222 += ("<tr><td>%s</td><td>%s</td><td style='word-break:break-all'>%s</td><td style='word-break:break-all'>%s</td><td>%s</td><td>%s</td><td style='word-break:break-all'>%s</td><td style='word-break:break-all'>%s</td><td><a href='%s'>url</a></td></tr>\n\r" %( x.code, x.name, x.method_version, x.log_code, x.debugmsg, x.error, x.message, pId, x.url))
		patterns = []
